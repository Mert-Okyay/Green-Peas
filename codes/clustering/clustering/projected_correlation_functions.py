...
from clustering.clustering.error import *
from Corrfunc.mocks.DDrppi_mocks import DDrppi_mocks

__author__ = "Meredith Powell <meredith.powell@yale.edu>"


def auto_wp(data, randoms, bins, pimax, m, estimator='L',cosmo=None,survey='BASS', JKN=True):
	'''
	...
	wp_err,cov=auto_jackknife(d=data,r=randoms,m=m,pimax=pimax,bins=bins,estimator=estimator,survey=survey, JKN=JKN)

	return rp,wp,wp_err,cov


def cross_wp(d1, d2, r2, bins, pimax, m, r1=None, estimator='L',cosmo=None,survey='BASS', paircounts=False, adaptive=0, JKN=True):
	'''
	...
	if (r1 is not None):
		if ('cdist' not in r1.dtype.names):
			r1 = np.array(z_to_cdist(r1,cosmo))
	...
	if paircounts:
		if (estimator=='L') or (estimator=='Landy'):
			wp, bins, d1d2, d1r2, d2r1, r1r2= wp_d1d2(d1=d1, d2=d2, r2=r2, bins=bins, pimax=pimax, r1=r1, estimator=estimator, paircounts=True, adaptive=adaptive)
		else:
			wp, bins, d1d2, d1r2= wp_d1d2(d1=d1, d2=d2, r2=r2, bins=bins, pimax=pimax, r1=r1, estimator=estimator, paircounts=True, adaptive=adaptive)
	else:
		wp, bins= wp_d1d2(d1=d1, d2=d2, r2=r2, bins=bins, pimax=pimax, r1=r1, estimator=estimator, adaptive=adaptive)
	rp=create_rp(bins)
	wp_err,cov=cross_jackknife(d1=d1,d2=d2,r1=r1,r2=r2,m=m,pimax=pimax,bins=bins,estimator=estimator,survey=survey, JKN=JKN)

	if paircounts:
		if (estimator=='L') or (estimator=='Landy'):
			return rp, wp, wp_err, cov, d1d2, d1r2, d2r1, r1r2
		else:
			return rp, wp, wp_err, cov, d1d2, d1r2
	else:
		return rp,wp,wp_err,cov
