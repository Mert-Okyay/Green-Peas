...
# Embedded file name: /Users/mcp74/Dropbox/clustering/code/clustering/randoms.py
# Compiled at: 2017-03-29 11:23:31
...
from clustering.clustering.kde import weighted_gaussian_kde
import random


def genrand(data, n, cosmo, width=0.2, useRaDec=True, scoords='galactic', use_BASS_sens_map=False, plot=True, plot_filename=None, randomize=True):
    """
    ...
    z_arr = data['z']
    if useRaDec is True:
        ra_arr = np.array(data['ra'], dtype=float)
        dec_arr = np.array(data['dec'], dtype=float)
        if use_BASS_sens_map is True:
            n_rand = int(round(n * len(data) * 1.2))
        else:
            n_rand = int(round(n * len(data)))
        ...
        if randomize is True:
            rarn_arr = np.random.rand(len(zr_arr)) * 360
            rar_arr = rarn_arr
            decrad = np.arcsin(np.random.rand(len(zr_arr)) * 2.0 - 1) * u.radian
        ...
        rcat = np.zeros((len(zr_arr)), dtype=[('z', '<f8'), ('ra', '<f8'), ('dec', '<f8'), ('l', '<f8'), ('b', '<f8')])
        rcat[:] = temp
        if use_BASS_sens_map is True:
    ...
    if useRaDec is False:
        ...
        if randomize is True:
            ...
            #rarn_arr = np.arcsin(np.random.rand(len(zr_arr)) * 2.0 - 1) * u.radian
            #rar_arr = rarn_arr.to(u.degree).value
            rar_arr =np.random.rand(len(zr_arr)) * 2.0 - 1
...
def BASS_sensitivity_filter(data, rcat):
    ...
    for i, r in enumerate(rcat):
        ...
        sens_map = smaps[sind]
        try:
            sensitivity = sens_map[(px, py)] * 2.39e-08 * 4.8
        except IndexError:
            print (l, b)
