...
def pair_count(rpbins,pimax,ra1,dec1,cd1,ra2=None,dec2=None,cd2=None,nthreads=4):
	...
	if ra2 is None:
		nn = DDrppi_mocks(autocorr=1, cosmology=1, nthreads=nthreads, pimax=pimax, binfile=rpbins, RA1=d1ra, DEC1=d1dec, CZ1=d1cd, is_comoving_dist=True, output_rpavg=True, verbose=True)
...
def wp_d1d2(d1, d2, r2, bins, pimax, r1=None, estimator='L',weights1=None,weights2=None, paircounts=False, adaptive=0):
	...
	#Check to see if pair count is greater than 7 per bin
	if adaptive!=0:
		print('adapting bins')
		#Establish median rp of bins
		rp=create_rp(bins)
		pc_arr, low = det_adapt(rp,pimax,d1d2,adaptive)
		if len(low)!=0:
			#remove the bin divider
			j=1
			newbins=[]
			newbins.append(bins[0])
			while j<len(bins):
				if not j==low[0]:
					newbins.append(bins[j])
				j+=1
			#rerun with new bins
			return wp_d1d2(d1, d2, r2, newbins, pimax, r1=r1, estimator=estimator,weights1=weights1,weights2=weights2, paircounts=paircounts, adaptive=adaptive)
	...
	if paircounts:
		if (estimator=='L') or (estimator=='Landy'):
			return wp, bins, d1d2, d1r2, d2r1, r1r2
		else:
			return wp, bins, d1d2, d1r2
	else:
		return wp, bins

def det_adapt(rp,pimax,d1d2,adaptive,pc_arr=None):
	if pc_arr==None:	
		#Sum paircounts per rp bin
		pc_arr=[]
		for i, r in enumerate(rp):
		    pc=0
		    for j in range(pimax-1):
		        pc+=d1d2['npairs'][i*pimax+j]
		    pc_arr.append(pc)
		pc_arr=np.array(pc_arr)
	#find bins with low pair counts
	low=[]
	for i, k in enumerate(pc_arr):
		if k<adaptive:
			low.append(i+1)
	low=np.array(low)
	return pc_arr, low
...
def create_rp(bins):
	...
	return bins[1:] - 0.5*np.array(rpwidths)
