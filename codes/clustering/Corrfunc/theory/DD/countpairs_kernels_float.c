...
#if defined(__AVX__)
#include "avx_calls.h"
static inline int countpairs_avx_intrinsics_float(const int64_t N0, float *x0, float *y0, float *z0, const weight_struct_float *weights0,
                                             ...
                                             float *src_weightavg, const weight_method_t weight_method)
{
  ...
  float rpavg[nbin], weightavg[nbin];
...
#pragma unroll(AVX_NVEC)
        ...
        for(int jj=0;jj<AVX_NVEC;jj++) {
          ...
          if(need_rpavg){
            const float r = union_mDperp.Dperp[jj];
            rpavg[kbin] += r;
          }
          if(need_weightavg){
    ...
    // remainder loop
    // pair.weights0[w].d was set as an AVX float, but is still valid as a float here
    for(;j<N1;j++){
      const float dz = *localz1++ - zpos;
...
static inline int countpairs_sse_intrinsics_float(const int64_t N0, float *x0, float *y0, float *z0, const weight_struct_float *weights0,
                                             const int64_t N1, float *x1, float *y1, float *z1, const weight_struct_float *weights1,
                                             const int same_cell,
                                             const float sqr_rpmax, const float sqr_rpmin, const int nbin, const float *rupp_sqr, const float rpmax,
                                             const float off_xwrap, const float off_ywrap, const float off_zwrap,
...
static inline int countpairs_fallback_float(const int64_t N0, float *x0, float *y0, float *z0, const weight_struct_float *weights0,
                                             const int64_t N1, float *x1, float *y1, float *z1, const weight_struct_float *weights1,
                                             const int same_cell,
                                             const float sqr_rpmax, const float sqr_rpmin, const int nbin, const float *rupp_sqr, const float rpmax,
                                             const float off_xwrap, const float off_ywrap, const float off_zwrap,
