/* This file is auto-generated from countpairs_impl.c.src */
#ifdef DOUBLE_PREC
#undef DOUBLE_PREC
#endif
// # -*- mode: c -*-
...
#include "countpairs_impl_float.h" //function proto-type
#include "countpairs_kernels_float.c"
...
#include "cellarray_float.h" //definition of struct cellarray*
#include "gridlink_impl_float.h"//function proto-type for gridlink

#if defined(_OPENMP)
#include <omp.h>
#endif

int interrupt_status_float=EXIT_SUCCESS;

void interrupt_handler_countpairs_float(int signo)
{
    fprintf(stderr,"Received signal = `%s' (signo = %d). Aborting \n",strsignal(signo), signo);
    interrupt_status_float = EXIT_FAILURE;
}    


countpairs_func_ptr_float countpairs_driver_float(const struct config_options *options)
{
    static countpairs_func_ptr_float function = NULL;
    ...
    countpairs_func_ptr_float allfunctions[] = {
#ifdef __AVX__
        countpairs_avx_intrinsics_float,
#endif			 
#ifdef __SSE4_2__
        countpairs_sse_intrinsics_float,
#endif
        countpairs_fallback_float
    };
...
int countpairs_float(const int64_t ND1, float *X1, float *Y1, float *Z1,
                      ...
                      struct extra_options *extra)
{
  if(options->float_type != sizeof(float)) {
    fprintf(stderr,"ERROR: In %s> Can only handle arrays of size=%zu. Got an array of size = %zu\n",
            __FUNCTION__, sizeof(float), options->float_type);
    return EXIT_FAILURE;
  }
  ...
  SETUP_INTERRUPT_HANDLERS(interrupt_handler_countpairs_float);
  ...
  get_max_min_float(ND1, X1, Y1, Z1, &xmin, &ymin, &zmin, &xmax, &ymax, &zmax);
  ...
  if(autocorr==0) {
    ...
    get_max_min_float(ND2, X2, Y2, Z2, &xmin, &ymin, &zmin, &xmax, &ymax, &zmax);
    if(options->verbose) {
      fprintf(stderr,"ND2 = %12"PRId64" [xmin,ymin,zmin] = [%lf,%lf,%lf], [xmax,ymax,zmax] = [%lf,%lf,%lf]\n",ND2,xmin,ymin,zmin,xmax,ymax,zmax);
    }
  }
  const float xdiff = options->boxsize > 0 ? options->boxsize:(xmax-xmin);
  const float ydiff = options->boxsize > 0 ? options->boxsize:(ymax-ymin);
  const float zdiff = options->boxsize > 0 ? options->boxsize:(zmax-zmin);
  const float pimax = (float) rpmax;
  ...
  cellarray_index_particles_float *lattice1 = gridlink_index_particles_float(ND1, X1, Y1, Z1, &(extra->weights0),
  ...
  if((max_nmesh <= BOOST_CELL_THRESH || avg_np >= BOOST_NUMPART_THRESH)
      ...
      if(get_bin_refine_scheme(options) == BINNING_DFL) {
          ...
          free_cellarray_index_particles_float(lattice1, nmesh_x * (int64_t) nmesh_y * nmesh_z);
          // Only boost the first two dimensions.  Prevents excessive refinement.
          for(int i=0;i<2;i++) {
              options->bin_refine_factors[i] += BOOST_BIN_REF;
          }
          lattice1 = gridlink_index_particles_float(ND1, X1, Y1, Z1, &(extra->weights0),
                                                     xmin, xmax, ymin, ymax, zmin, zmax,
                                                     rpmax, rpmax, rpmax,
                                                     options->bin_refine_factors[0], options->bin_refine_factors[1], options->bin_refine_factors[2],
                                                     &nmesh_x, &nmesh_y, &nmesh_z, options);
  ...
  cellarray_index_particles_float *lattice2 = NULL;
    if(autocorr==0) {
        int ngrid2_x=0,ngrid2_y=0,ngrid2_z=0;
        lattice2 = gridlink_index_particles_float(ND2, X2, Y2, Z2, &(extra->weights1),
                                                   xmin, xmax, ymin, ymax, zmin, zmax,
                                                   rpmax, rpmax, rpmax,
                                                   options->bin_refine_factors[0], options->bin_refine_factors[1], options->bin_refine_factors[2],
    ...
    {
        int status = assign_ngb_cells_index_particles_float(lattice1, lattice2, totncells,
                                                             options->bin_refine_factors[0], options->bin_refine_factors[1], options->bin_refine_factors[2],
                                                             nmesh_x, nmesh_y, nmesh_z, xdiff, ydiff, zdiff,
                                                             autocorr, options->periodic);
        if(status != EXIT_SUCCESS) {
            free_cellarray_index_particles_float(lattice1, totncells);
            if(autocorr == 0) {
                free_cellarray_index_particles_float(lattice2, totncells);
            }
            free(rupp);
            return status;
        }
    ...
    countpairs_func_ptr_float countpairs_function_float = countpairs_driver_float(options);
    if(countpairs_function_float == NULL) {
        free_cellarray_index_particles_float(lattice1, totncells);
        if(autocorr == 0) {
            free_cellarray_index_particles_float(lattice2, totncells);
        }
    ...
    float **all_rpavg = NULL;
    if(options->need_avg_sep) {
      all_rpavg = (float **) matrix_calloc(sizeof(float),numthreads,nrpbin);
    }
    
    float **all_weightavg = NULL;
    if(need_weightavg) {
      all_weightavg = (float **) matrix_calloc(sizeof(float),numthreads,nrpbin);
    }
    ...
    float rupp_sqr[nrpbin];
    for(int i=0; i < nrpbin;i++) {
      rupp_sqr[i] = rupp[i]*rupp[i];
    }

    float sqr_rpmax=rupp_sqr[nrpbin-1];
    float sqr_rpmin=rupp_sqr[0];
...
#if defined(_OPENMP)
#pragma omp parallel shared(numdone, abort_status, interrupt_status_float)
    {
      int tid = omp_get_thread_num();
      uint64_t npairs[nrpbin];
      float rpavg[nrpbin]; //thread-level, stored on stack
      float weightavg[nrpbin];
...
#if defined(_OPENMP)
#pragma omp flush (abort_status, interrupt_status_float)
#endif
        if(abort_status == EXIT_SUCCESS && interrupt_status_float == EXIT_SUCCESS) { 
...
#pragma omp atomic
          ...
          const cellarray_index_particles_float *first  = &(lattice1[index1]);
          ...
          float *y1 = first->y;
          float *z1 = first->z;
          const weight_struct_float *weights1 = &(first->weights);
          const int64_t N1 = first->nelements;
          if(autocorr == 1) {
              int same_cell = 1;
              float *this_rpavg = NULL;
              float *this_weightavg = NULL;
              if(options->need_avg_sep) {
                  this_rpavg = rpavg;
              }
              if(need_weightavg) {
              ...
              const int status = countpairs_function_float(N1, x1, y1, z1, weights1,
                                                            N1, x1, y1, z1, weights1,
                                                            same_cell,
                                                            sqr_rpmax, sqr_rpmin, nrpbin, rupp_sqr, pimax, //pimax is simply rpmax cast to float
                                                            ZERO, ZERO, ZERO,
                                                            this_rpavg, npairs,
          ...
          for(int64_t ngb=0;ngb<first->num_ngb;ngb++){
            const cellarray_index_particles_float *second = first->ngb_cells[ngb];
            ...
            float *y2 = second->y;
            float *z2 = second->z;
            const weight_struct_float *weights2 = &(second->weights);
            float off_xwrap = 0.0, off_ywrap = 0.0, off_zwrap = 0.0;
            if(options->periodic) {
          ...
          /*posix_madvise(first->x, sizeof(float)*N1, MADV_DONTNEED);
          posix_madvise(first->y, sizeof(float)*N1, MADV_DONTNEED);
          posix_madvise(first->z, sizeof(float)*N1, MADV_DONTNEED);*/
        }//abort-status
    ...
    if(autocorr == 1) {
      ...
      if(rupp[0] <= 0.0) {
          ...
          if(need_weightavg){
            ...
            weight_func_t_float weight_func = get_weight_func_by_method_float(extra->weight_method);
            ...
            for(int j = 0; j < ND1; j++){
                for(int w = 0; w < pair.num_weights; w++){
                    pair.weights0[w].d = ((float *) extra->weights0.weights[w])[j];
                    pair.weights1[w].d = ((float *) extra->weights0.weights[w])[j];
                }
  ...
  for(int i=0;i<nrpbin;i++) {
    if(npairs[i] > 0) {
      if(options->need_avg_sep) {
        rpavg[i] /= (float) npairs[i] ;
      }
      if(need_weightavg) {
        weightavg[i] /= (float) npairs[i];
      }
    }
  }
