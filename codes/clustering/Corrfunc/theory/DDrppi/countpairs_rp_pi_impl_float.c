/* This file is auto-generated from countpairs_rp_pi_impl.c.src */
#ifdef DOUBLE_PREC
#undef DOUBLE_PREC
#endif
// # -*- mode: c -*-
...
#include "countpairs_rp_pi_impl_float.h" //function proto-type
#include "countpairs_rp_pi_kernels_float.c"
...
int interrupt_status_DDrppi_float=EXIT_SUCCESS;

void interrupt_handler_countpairs_rp_pi_float(int signo)
{
    fprintf(stderr,"Received signal = `%s' (signo = %d). Aborting \n",strsignal(signo), signo);
    interrupt_status_DDrppi_float = EXIT_FAILURE;
}    

countpairs_rp_pi_func_ptr_float countpairs_rp_pi_driver_float(const struct config_options *options)
{
    static countpairs_rp_pi_func_ptr_float function = NULL;
    ...
    countpairs_rp_pi_func_ptr_float allfunctions[] = {
#ifdef __AVX__
      countpairs_rp_pi_avx_intrinsics_float,
#endif			 
#ifdef __SSE4_2__
      countpairs_rp_pi_sse_intrinsics_float,
#endif
      countpairs_rp_pi_fallback_float
    };
...
int countpairs_rp_pi_float(const int64_t ND1, float *X1, float *Y1, float *Z1,
                            ...
                            struct extra_options *extra)
{
    ...
    SETUP_INTERRUPT_HANDLERS(interrupt_handler_countpairs_rp_pi_float);
    ...
    const float sqr_rpmax=rupp_sqr[nrpbin-1];
    const float sqr_rpmin=rupp_sqr[0];
    ...
    countpairs_rp_pi_func_ptr_float countpairs_rp_pi_function_float = countpairs_rp_pi_driver_float(options);
    if(countpairs_rp_pi_function_float == NULL) {
        free_cellarray_index_particles_float(lattice1, totncells);
        if(autocorr == 0) {
            free_cellarray_index_particles_float(lattice2, totncells);
        }
    ...
    if(options->need_avg_sep) {
        all_rpavg = (float **) matrix_calloc(sizeof(float),numthreads,totnbins);
    }
    float **all_weightavg = NULL;
    if(need_weightavg) {
      all_weightavg = (float **) matrix_calloc(sizeof(float),numthreads,totnbins);
    }
    ...
    float rpavg[totnbins], weightavg[totnbins];
...
#if defined(_OPENMP)
#pragma omp parallel shared(numdone, abort_status, interrupt_status_DDrppi_float)
    {
...
#if defined(_OPENMP)
#pragma omp flush (abort_status, interrupt_status_DDrppi_float)
#endif
            if(abort_status == EXIT_SUCCESS && interrupt_status_DDrppi_float == EXIT_SUCCESS) {
...
#pragma omp atomic
                ...
                if(autocorr == 1) {
                    ...
                    const int status = countpairs_rp_pi_function_float(N1, x1, y1, z1, weights1,
...
#if defined(_OPENMP)
    uint64_t npairs[totnbins];
    float rpavg[totnbins];
    float weightavg[totnbins];
