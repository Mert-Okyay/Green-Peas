/* This file is auto-generated from countpairs_rp_pi_kernels.c.src */
#ifdef DOUBLE_PREC
#undef DOUBLE_PREC
#endif
// # -*- mode: c -*-
...
#include "weight_functions_float.h"

#if defined(__AVX__)
#include "avx_calls.h"

static inline int countpairs_rp_pi_avx_intrinsics_float(const int64_t N0, float *x0, float *y0, float *z0, const weight_struct_float *weights0,
                                                         const int64_t N1, float *x1, float *y1, float *z1, const weight_struct_float *weights1, const int same_cell, 
                                                         const float sqr_rpmax, const float sqr_rpmin, const int nbin,
                                                         const int npibin, const float *rupp_sqr, const float pimax,
                                                         ...
                                                         float *src_weightavg, const weight_method_t weight_method)
{
    ...
    for(int i=0;i<nbin;i++) {
        ...
        m_kbin[i] = AVX_SET_FLOAT((float) i);
    }

    const float dpi = pimax/npibin;
    const float inv_dpi = 1.0/dpi;
    
    // A copy whose pointers we can advance
    weight_struct_float local_w0 = {.weights={NULL}, .num_weights=0}, 
                         local_w1 = {.weights={NULL}, .num_weights=0};
    pair_struct_float pair = {.num_weights=0};
    avx_weight_func_t_float avx_weight_func = NULL;
    weight_func_t_float fallback_weight_func = NULL;
    if(need_weightavg){
        ...
        avx_weight_func = get_avx_weight_func_by_method_float(weight_method);
        fallback_weight_func = get_weight_func_by_method_float(weight_method);
    }

    int64_t prev_j = 0, n_off = 0;    
    for(int64_t i=0;i<N0;i++) {
        const float xpos = *x0++ + off_xwrap;
        const float ypos = *y0++ + off_ywrap;
        const float zpos = *z0++ + off_zwrap;
        ...
        float *localz1 = z1;
        float *localx1 = x1 + n_off;
        float *localy1 = y1 + n_off;
        ...
        for(;j<=(N1 - AVX_NVEC);j+=AVX_NVEC) {
            ...
            const AVX_FLOATS m_pimax = AVX_SET_FLOAT((float) pimax);
            ...
            const AVX_FLOATS m_npibin     = AVX_SET_FLOAT((float) npibin);
            const AVX_FLOATS m_one    = AVX_SET_FLOAT((float) 1);
            ...
            AVX_FLOATS m_rpbin     = AVX_SET_FLOAT((float) 0);
...
#pragma unroll(AVX_NVEC)
            ...
            for(int jj=0;jj<AVX_NVEC;jj++) {
                ...
                if(need_weightavg){
                    const float weight = union_mweight.weights[jj];
        ...
        //remainder loop 
        for(;j<N1;j++){
            const float dz = FABS(*localz1++ - zpos);
            const float dx = *localx1++ - xpos;
            const float dy = *localy1++ - ypos;
            for(int w = 0; w < pair.num_weights; w++){
...
static inline int countpairs_rp_pi_sse_intrinsics_float(const int64_t N0, float *x0, float *y0, float *z0, const weight_struct_float *weights0,
                                                         const int64_t N1, float *x1, float *y1, float *z1, const weight_struct_float *weights1, const int same_cell,
                                                         ...
                                                         float *src_weightavg, const weight_method_t weight_method)
{
    ...
    for(int i=0;i<nbin;i++) {
        m_kbin[i] = SSE_SET_FLOAT((float) i);
    }
    ...
    weight_struct_float local_w0 = {.weights={NULL}, .num_weights=0}, 
                         local_w1 = {.weights={NULL}, .num_weights=0};
    pair_struct_float pair = {.num_weights=0};
    sse_weight_func_t_float sse_weight_func = NULL;
    weight_func_t_float fallback_weight_func = NULL;
    if(need_weightavg){
      // Same particle list, new copy of num_weights pointers into that list
      local_w0 = *weights0;
      ...
      sse_weight_func = get_sse_weight_func_by_method_float(weight_method);
      fallback_weight_func = get_weight_func_by_method_float(weight_method);
    }
    ...
    for(int64_t i=0;i<N0;i++) {
        ...
        for(;j<=(N1 - SSE_NVEC);j+=SSE_NVEC){
            ...
            union float4_weights{
                ...
                float weights[SSE_NVEC];
            };
            union float4_weights union_mweight;
            
            const SSE_FLOATS m_pimax = SSE_SET_FLOAT((float) pimax);
            ...
            const SSE_FLOATS m_npibin     = SSE_SET_FLOAT((float) npibin);
            const SSE_FLOATS m_one    = SSE_SET_FLOAT((float) 1);
            ...
            SSE_FLOATS m_rpbin     = SSE_SET_FLOAT((float) 0);
...
static inline int countpairs_rp_pi_fallback_float(const int64_t N0, float *x0, float *y0, float *z0, const weight_struct_float *weights0,
                                                   ...
                                                   float *src_weightavg, const weight_method_t weight_method)
{
    ...
    weight_struct_float local_w0 = {.weights={NULL}, .num_weights=0}, 
                         local_w1 = {.weights={NULL}, .num_weights=0};
    pair_struct_float pair = {.num_weights=0};
    weight_func_t_float weight_func = NULL;
    if(need_weightavg){
        // Same particle list, new copy of num_weights pointers into that list
        local_w0 = *weights0;
        local_w1 = *weights1;
        ...
        weight_func = get_weight_func_by_method_float(weight_method);
    }
