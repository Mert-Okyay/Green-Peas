...
int get_binsize_float(const float xmin,const float xmax, const float rmax, const int refine_factor, const int max_ncells, float *xbinsize, int *nlattice, const struct config_options *options)
{
...
void free_cellarray_float(cellarray_float *lattice, const int64_t totncells)
{
...
void free_cellarray_index_particles_float(cellarray_index_particles_float *lattice, const int64_t totncells)
{
...
cellarray_float * gridlink_float(const int64_t np,
                                   ...
                                   const struct config_options *options)
{
    ...
    float xbinsize=ZERO, ybinsize=ZERO, zbinsize=ZERO;
    const int xstatus = get_binsize_float(xmin,xmax,max_x_size,xbin_refine_factor, options->max_cells_per_dim, &xbinsize, &nmesh_x, options);
    const int ystatus = get_binsize_float(ymin,ymax,max_y_size,ybin_refine_factor, options->max_cells_per_dim, &ybinsize, &nmesh_y, options);
    const int zstatus = get_binsize_float(zmin,zmax,max_z_size,zbin_refine_factor, options->max_cells_per_dim, &zbinsize, &nmesh_z, options);
    ...
    const float xdiff = (options->periodic && options->boxsize > 0) ? options->boxsize:xmax-xmin;
    const float ydiff = (options->periodic && options->boxsize > 0) ? options->boxsize:ymax-ymin;
    const float zdiff = (options->periodic && options->boxsize > 0) ? options->boxsize:zmax-zmin;

    const float cell_volume=xbinsize*ybinsize*zbinsize;
    const float box_volume=xdiff*ydiff*zdiff;
    ...
    cellarray_float *lattice = (cellarray_float *) my_malloc(sizeof(*lattice), totncells);
    ...
    */
    for (int64_t index=0;index<totncells;index++) {
        const size_t memsize=sizeof(float);
    ...
    const float xinv=1.0/xbinsize;
    const float yinv=1.0/ybinsize;
    const float zinv=1.0/zbinsize;
        ...
        if(lattice[index].nelements == nallocated[index]) {
            ...
            if(expected_n == nallocated[index]) {
                ...
                fprintf(stderr,"In %s> Reallocation failed,  randomly subsampling the input particle set (currently at %"PRId64" particles) might help\n",
                        __FUNCTION__, np);
                free_cellarray_float(lattice, totncells);
                free(nallocated);
                return NULL;
            }
            nallocated[index] = expected_n;
    ...
    //You can free the extra memory reserved by the mallocs by looping over totncells and doing a realloc(lattice[index].x,sizeof(float),lattice[index].nelements,"lattice.x")
...
cellarray_index_particles_float * gridlink_index_particles_float(const int64_t np,
                                                                   ...
                                                                   const struct config_options *options)
{
    ...
    cellarray_index_particles_float *lattice  = (cellarray_index_particles_float *) my_malloc(sizeof(*lattice), totncells);
    int64_t *nallocated = (int64_t *)       my_malloc(sizeof(*nallocated)      , totncells);
    if(lattice == NULL || nallocated == NULL) {
        free(lattice);free(nallocated);
        return NULL;
    ...
    for (int64_t index=0;index<totncells;index++) {
        ...
        for(int w = 0; w < lattice[index].weights.num_weights; w++){
          lattice[index].weights.weights[w] = (float *) my_malloc(memsize, expected_n);
    ...
    for (int64_t i=0;i<np;i++)  {
        ...
        if(lattice[index].nelements == nallocated[index]) {
            ...
            do {
                ...
                for(int w = 0; w < lattice[index].weights.num_weights; w++){
                  float *newweights = (float *) my_realloc(lattice[index].weights.weights[w], memsize, expected_n, "lattice.weights");
                  if(newweights == NULL){
                    w_alloc_status = EXIT_FAILURE;
                  } else {
            ...
            if(expected_n == nallocated[index]) {
                ...
                fprintf(stderr,"In %s> Reallocation failed,  randomly subsampling the input particle set (currently at %"PRId64" particles) might help\n",
                        __FUNCTION__, np);
                free_cellarray_index_particles_float(lattice, totncells);
                free(nallocated);
                return NULL;
            }
            nallocated[index] = expected_n;
        ...
        lattice[index].z[ipos] = z[i];
        for(int w = 0; w < lattice[index].weights.num_weights; w++){
            lattice[index].weights.weights[w][ipos] = ((float *)weights->weights[w])[i];
        }
        lattice[index].nelements++;
    }
    free(nallocated);
...
#define MULTIPLE_ARRAY_EXCHANGER(type,a,i,j) { SGLIB_ARRAY_ELEMENTS_EXCHANGER(float,X,i,j);        \
                                               ...
                                               for(int w = 0; w < first->weights.num_weights; w++){ \
                                                 SGLIB_ARRAY_ELEMENTS_EXCHANGER(float,first->weights.weights[w],i,j);\
                                               }\
                                             }

            const cellarray_index_particles_float *first=&(lattice[icell]);
            ...
            SGLIB_ARRAY_QUICK_SORT(float, Z, first->nelements, SGLIB_NUMERIC_COMPARATOR , MULTIPLE_ARRAY_EXCHANGER);
#undef MULTIPLE_ARRAY_EXCHANGER
    ...
    //and doing a realloc(lattice[index].x,sizeof(float),lattice[index].nelements,"lattice.x")
...
int assign_ngb_cells_index_particles_float(struct cellarray_index_particles_float *lattice1, struct cellarray_index_particles_float *lattice2, const int64_t totncells,
                                             ...
                                             const int autocorr, const int periodic)
{
  ...
  for(int64_t icell=0;icell<totncells;icell++) {
    struct cellarray_index_particles_float *first = &(lattice1[icell]);
    if(first->nelements == 0) continue;
    ...
    for(int iix=-xbin_refine_factor;iix<=xbin_refine_factor;iix++){
      ...
      const float off_xwrap = ((ix + iix) >= 0) && ((ix + iix) < nmesh_x) ? 0.0: ((ix+iix) < 0 ? xdiff:-xdiff);
      ...
      for(int iiy=-ybin_refine_factor;iiy<=ybin_refine_factor;iiy++) {
        ...
        const float off_ywrap = ((iy + iiy) >= 0) && ((iy + iiy) < nmesh_y) ? 0.0: ((iy+iiy) < 0 ? ydiff:-ydiff);
          ...
          for(int64_t iiz=start_iz;iiz<=zbin_refine_factor;iiz++){
            ...
            const float off_zwrap = ((iz + iiz) >= 0) && ((iz + iiz) < nmesh_z) ? 0.0: ((iz+iiz) < 0 ? zdiff:-zdiff);
